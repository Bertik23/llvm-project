...
export class LLVMContext implements vscode.Disposable {
  ...
  async getOrActivateLanguageClient(uri: vscode.Uri, languageId: string):
    ...
    let serverBinaryName: string;
    if (languageId === 'llvm') {
      serverBinaryName = 'llvm-lsp-server';
    } else {
      return null;
    }
    ...
    if (!client) {
      client = await this.activateWorkspaceFolder(
        workspaceFolder, serverBinaryName, languageId, this.outputChannel);
  ...
  async activateWorkspaceFolder(workspaceFolder: vscode.WorkspaceFolder,
    serverBinaryName: string, languageName: string,
    outputChannel: vscode.OutputChannel):
    Promise<vscodelc.LanguageClient> {
    let configsToWatch: string[] = [];
    let filepathsToWatch: string[] = [];
    let additionalServerArgs: string[] = [];
    additionalServerArgs =  [`--opt-path=${config.getInstallPath()}opt`].concat(config.get<string[]>("additionalServerArgs", null, []));

    // Try to activate the language client.
    const [server, serverPath] = await this.startLanguageClient(
      workspaceFolder, outputChannel, serverBinaryName, languageName,
      additionalServerArgs);
    configsToWatch.push('installPath');
  ...
  async startLanguageClient(workspaceFolder: vscode.WorkspaceFolder,
    ...
    // functionality.
    var serverPath =
      await this.resolveServerPath(serverBinaryName, workspaceFolder);
    ...
    if (!fs.existsSync(serverPath)) {
      vscode.window
        .showErrorMessage(
          `${clientTitle}: Unable to resolve path for '${serverBinaryName}', please ensure the path is correct`,
          "Open Setting")
        .then((value) => {
          if (value === "Open Setting") {
            vscode.commands.executeCommand(
              'workbench.action.openWorkspaceSettings',
              { openToSide: false, query: `llvm.${serverBinaryName}` });
          }
        });
      return [null, serverPath];
    }
   ...
   * workspace folder.
   */
  async resolveServerPath(serverBinaryName: string,
    workspaceFolder: vscode.WorkspaceFolder):
    Promise<string> {
    const serverPath = config.getInstallPath(workspaceFolder) + `${serverBinaryName}`;
    return this.resolvePath(serverPath, workspaceFolder);
  }
