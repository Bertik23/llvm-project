...
public:
  ...
  // Sends diagnostics
  void sendDiagnostics(const lsp::URIForFile &File,
...
private:
  ...
  void handleNotificationTextDocumentDidChange(
      const lsp::DidChangeTextDocumentParams &Params);

  // textDocument/didSave
  void handleNotificationTextDocumentDidSave(
  ...
  // textDocument/hover
  void handleRequestTextDocumentHover(
      const lsp::TextDocumentPositionParams &Params,
  ...
  // llvm/runPassOnIR
  void handleRequestRunPassOnIR(const lsp::RunPassOnIRParams &Params,
                                lsp::Callback<lsp::IR> Reply);

  // Identifies RPC Call and dispatches the handling to other methods
  bool registerMessageHandlers();

  lsp::OutgoingNotification<lsp::PublishDiagnosticsParams>
