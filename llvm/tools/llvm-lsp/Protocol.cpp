...
bool llvm::lsp::fromJSON(const llvm::json::Value &Value,
  ...
  return O && O.map("uri", Result.uri) && O.map("pipeline", Result.pipeline) &&
         ...
         O.map("pipeline_opt_args",
               Result.pipeline_opt_args) &&
         O.map("pass_opt_args",
               Result.pass_opt_args);
}
...
bool llvm::lsp::fromJSON(const llvm::json::Value &Value,
                         RunPassOnIRParams &Result, llvm::json::Path Path) {
  llvm::json::ObjectMapper O(Value, Path);
  return O && O.map("uri", Result.uri) && O.map("pass", Result.pass) &&
         O.map("pipeline_opt_args", Result.pipeline_opt_args) &&
         O.map("pass_opt_args", Result.pass_opt_args) &&
         O.map("stderr_path", Result.stderr_path);
}
